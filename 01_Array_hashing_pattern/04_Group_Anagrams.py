...
# frequency. Instead of sorting each word (O(n log n)), we build
# a fixed-size frequency array of length 26 (one slot per a–z).
# This array, converted to a tuple, becomes a hashable key that
# groups all anagrams together in a dictionary.
#
...
# "eat" → count → (1,0,0,0,1,0,…,1,…) → key_A
# "tea" → same count                   → key_A  ✓ grouped
# "ate" → same count                   → key_A  ✓ grouped
# "tan" → different count              → key_B
# "nat" → same as "tan"                → key_B  ✓ grouped
# "bat" → different count              → key_C
#
